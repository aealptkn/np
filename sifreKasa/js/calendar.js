const CalendarApp = {
    ...
    loadItems() {
        if (typeof AppState !== 'undefined' && AppState.decryptedItems) {
            ...
            if (!document.getElementById("calYearView").classList.contains("hidden")) this.renderYear();
            else if (!document.getElementById("calWeekView").classList.contains("hidden")) this.renderWeek();
            else if (!document.getElementById("calListView").classList.contains("hidden")) this.renderAllList();
    ...
    // --- SES DİNLEME VE ALARM MOTORU ---
    previewSound(src) {
        if (!src || src === 'silent') return;
        try { const audio = new Audio(src); audio.play(); } catch (e) {}
    },
    ...
    showCustomAlert(item, callback) {
        ...
        // Ertele Butonu Sadece Etkinliklerde veya Snooze Açık Alarmlarda Görünür
        let snoozeBtnHtml = '';
        ...
        if (canSnooze) {
            box.querySelector("#btnSnooze").onclick = () => { document.body.removeChild(overlay); callback('snooze'); };
        }
        
        box.querySelector("#btnOk").onclick = () => { document.body.removeChild(overlay); callback('ok'); };
